 ...
 */
public class ListOfRealPropertyPage implements IBookkeepingPageFactory {
	...
	public void createPages(AccountEditor editor, IEditorInput input,
		 ...
		 */
        if (memento != null) {
        	for (IMemento stockMemento : memento.getChildren("realproperty")) {
        		String propertyName = stockMemento.getString("name");
        		if (propertyName != null) {
        			for (Commodity commodity: sessionManager.getSession().getCommodityCollection()) {
        				if (commodity instanceof RealProperty) {
        					RealProperty property = (RealProperty)commodity;
        					if (property.getName().equals(propertyName)) {
        						IEditorPart stockDetailsEditor = new AssetDetailsEditor(property);
        						editor.addPage(stockDetailsEditor, property.getName());
        					}
        				}
        			}
        		}
		...
		IHandler handler = new ShowPropertyDetailsHandler(editor);
