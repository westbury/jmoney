 ...
 */
public class StockEntryFacadeB implements EntryFacade {
	...
	public StockEntryFacadeB(Entry netAmountEntry, StockAccount stockAccount) {
	...
	private void analyzeTransaction() {
		...
		if (getMainEntry().getTransaction().getEntryCollection().size() == 1) {
			...
			for (Entry entry: getMainEntry().getTransaction().getEntryCollection()) {
				String[] values = entry.getType().split(",");
				boolean conflictFound = false;
				for (String value : values) {
					String[] parts = value.split(":");
					if (parts[0].startsWith("stock.")) {
						Optional<TransactionType> matchingType = Arrays.asList(TransactionType.values()).stream().filter(eachType -> eachType.getId().equals(parts[0])).findAny();
				...
				if (newType != null && !conflictFound) {
					...
					switch(newType) {
					case Dividend:
						this.facade = new StockDividendFacade(getMainEntry().getTransaction(), "", account);
					}
				} else {
					this.transactionType.setValue(null);
					this.facade = null;
	...
	private void forceTransactionToTransfer() {
		...
		this.forceTransactionTo(TransactionType.Transfer);
	}
	...
	public StockDividendFacade getDividendFacade() {
		Assert.isTrue(isDividend());
		return new StockDividendFacade(getMainEntry().getTransaction(), "", account);
	}
