...
/**
 * A wizard to import data from multiple files.  An example would be to select one
 * or more bank statements stored as PDF files.  One PDF file may contain details for
 * more than one account (typical in the US).
 * <P>
 * This wizard is a single page wizard.
 */
public abstract class PdfImportWizard<T extends BaseEntryData> extends Wizard {

	protected IWorkbenchWindow window;

	protected PdfImportWizardPage mainPage;
	 ...
	 */
	public PdfImportWizard() {
		IDialogSettings workbenchSettings = Activator.getDefault().getDialogSettings();
		IDialogSettings section = workbenchSettings.getSection("PdfImportWizard");//$NON-NLS-1$
		if (section == null) {
			section = workbenchSettings.addNewSection("PdfImportWizard");//$NON-NLS-1$
		}
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		mainPage = new PdfImportWizardPage(window, getDescription());
